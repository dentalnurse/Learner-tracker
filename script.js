...
// ── UI HELPERS ───────────────────────
function isMarkedThisWeek(lastMarked) {
  if (!lastMarked) return false;
  return lastMarked > (Date.now() - (7 * 24 * 60 * 60 * 1000));
}
...
function learnerBar(id, curr, fn) {
  ...
  el.innerHTML = DB.learners.map((l, i) => {
    const isDone = isMarkedThisWeek(l.lastMarked) ? 'marked-done' : '';
    return `<button class="lpill ${i===curr?'active':''} ${isDone}" onclick="${fn}(${i})"><span class="lpill-dot"></span>${l.name}</button>`;
  }).join('');
}
...
function switchTab(name, btn) {
  ...
  activeEditIdx = null; // Reset edit form when changing tabs
...
function renderDashboard() {
  ...
  const percent = l.acs.length > 0 ? Math.round((done / l.acs.length) * 100) : 0;
  const needsMarking = !isMarkedThisWeek(l.lastMarked);
  ...
  el.innerHTML = `
    ...
    <div class="profile-card">
      ...
      <div class="profile-info">
        <div class="profile-name">${l.name} ${needsMarking ? '<span class="sbdg s-amend" style="margin-left:8px;font-size:10px">⚠️ MARKING REQUIRED</span>':'<span class="sbdg s-done" style="margin-left:8px;font-size:10px">✅ UP TO DATE</span>'}</div>
...
function renderEdit() {
  ...
  const listHTML = DB.learners.map((l, i) => `
    <div class="table-card" style="padding:15px; margin-bottom:12px; display:flex; justify-content:space-between; align-items:center;">
      <div style="display:flex; align-items:center; gap:15px;">
        <div class="avatar" style="width:40px; height:40px; font-size:14px;">${initials(l.name)}</div>
        <div><div style="font-weight:600;">${l.name}</div><div style="font-size:12px; color:var(--ink3)">${l.cohort} • ${l.type.toUpperCase()}</div></div>
      </div>
      <div style="display:flex; gap:8px;">
        <button class="lpill" onclick="openEditForm(${i})">Edit Details</button>
        <button class="lpill" style="color:var(--red); border-color:var(--red);" onclick="deleteLearner(${i})">Delete</button>
      </div>
    </div>`).join('');

  const formHTML = activeEditIdx !== null ? `
    <div class="table-card" style="padding:25px; margin-top:20px; border-top: 4px solid var(--blue); animation: pagein 0.3s ease;">
      <h3 style="font-family:'Fraunces',serif; margin-bottom:20px;">Edit Profile: ${DB.learners[activeEditIdx].name}</h3>
      <div class="form-row"><label>Full Name</label><input id="edit-name" class="tt-edit-input" style="width:100%" type="text" value="${DB.learners[activeEditIdx].name}"></div>
      <div class="form-row"><label>Cohort</label><input id="edit-cohort" class="tt-edit-input" style="width:100%" type="text" value="${DB.learners[activeEditIdx].cohort}"></div>
      <div style="margin-top:25px; display:flex; gap:10px;">
        <button class="btn-save" onclick="saveEditForm()">Save Changes</button>
        <button class="lpill" onclick="activeEditIdx=null; renderEdit()">Cancel</button>
      </div>
    </div>` : '';
...
// ── REMAINING LOGIC (TIMETABLE, COURSES, DRAG) ───────────────────────
function renderTimetable() {
  const el = document.getElementById('tab-timetable');
  const l = DB.learners[cTT];
  if(!l) { el.innerHTML = '<div class="empty">No learners yet.</div>'; return; }
  const rows = l.timetable.map((t, i) => `<tr><td style="text-align:center; font-weight:bold; color:var(--ink3)">${i + 1}</td><td style="width:45%"><div style="font-weight:600;">${t.label}</div><div style="font-size:11px; color:var(--ink3)">${t.reqs||''}</div></td><td><input type="text" class="tt-edit-input" style="width:100%" value="${t.date||''}" onchange="DB.learners[${cTT}].timetable[${i}].date=this.value;save();"></td></tr>`).join('');
  el.innerHTML = `<div class="page-header"><div class="page-title">Timetable</div></div><div class="learner-bar" id="tt-btns"></div><div class="table-card"><table class="tbl"><tbody>${rows}</tbody></table></div>`;
...
function handleDragOver(e) { e.preventDefault(); if (e.clientY < 100) window.scrollBy(0, -10); else if (window.innerHeight - e.clientY < 100) window.scrollBy(0, 10); }
